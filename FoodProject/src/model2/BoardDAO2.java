...
public class BoardDAO2 {
	...
	public void insertBoard(BoardBean2 bean) {
		...
		try {
			...
			String refsql = "select max(ref) from board2";	//ref의 값중 가장 큰값을 리턴해주세요
			...
			String sql = "insert into board2 values(board2_seq.NEXTVAL,?,?,?,?,?,sysdate,?,?,?,0,?)";	//board_seq.NEXTVAL(시퀀스)시퀀스에 들어가있는 값의 다음값을 넣어준다
	...
	public Vector<BoardBean2> getAllBoard(int start,int end) {
		...
		try {
			...
			while(rs.next()) {
				...
				bean.setReg_date(rs.getDate(7).toString());	//뒤에 toString을 붙여 reg_date를 스트링으로 변환
	...
	public void reWriteBoard(BoardBean2 bean) {
		...
		try {
			...
			pstmt.setInt(7, re_step+1);		//답글이기에 부모 step에 1을 더해 줌
			pstmt.setInt(8, re_level+1);	//답글이기에 부모level에 1을 더해줌
			pstmt.setString(9, bean.getContent());
